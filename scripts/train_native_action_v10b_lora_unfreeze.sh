...
OUTPUT_DIR="${OUTPUT_DIR:-training/native_action_v10b_lora_unfreeze_600}"
TRAIN_STEPS="${TRAIN_STEPS:-600}"
BATCH_SIZE="${BATCH_SIZE:-1}"
CHECKPOINTING_STEPS="${CHECKPOINTING_STEPS:-200}"

# Same as v10b baseline except --freeze_lora false to actually train LoRA
accelerate launch --config_file configs_acc/1gpu_smoke.yaml finetune.py \
  ...
  --train_epochs 20 --train_steps "$TRAIN_STEPS" --seed 42 \
  --batch_size "$BATCH_SIZE" --gradient_accumulation_steps 1 --mixed_precision bf16 \
  --num_workers 0 --pin_memory true \
  --checkpointing_steps "$CHECKPOINTING_STEPS" --checkpointing_limit 4 \
  --do_validation false \
  --validation_dir "$TRAIN_MANIFEST" \
  --cache_dir data --control_type add \
  --learning_rate 5e-5 --control_lr 5e-5 --native_projection_lr 2e-5 \
  --max_grad_norm 1.0 --lr_scheduler cosine --lr_warmup_steps 50 \
