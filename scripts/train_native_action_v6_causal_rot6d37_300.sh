...
export MASTER_PORT="${MASTER_PORT:-29728}"
...
accelerate launch --config_file configs_acc/1gpu_smoke.yaml finetune.py \
  ...
  --freeze_lora true \
  --condition_mode native_trajectory --native_conditioner_version v6 \
  --native_trajectory_dim 37 \
  --native_baseline_init reports/direct_action/gate_d/run_015/checkpoints/adapter_step1000.pt \
  --native_adapter_init training/native_action_v3_frozenbaseline_rot6d37_500/checkpoint-300 \
  ...
  --output_dir training/native_action_v6_causal_rot6d37_300 \
  --report_to tensorboard --train_resolution 33x480x832 \
  --train_epochs 100 --train_steps 300 --seed 42 \
  ...
  --learning_rate 1e-4 --control_lr 1e-4 --max_grad_norm 1.0 \
