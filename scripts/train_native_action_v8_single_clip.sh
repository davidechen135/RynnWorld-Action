...
export MASTER_PORT="${MASTER_PORT:-29739}"

TRAIN_MANIFEST="${TRAIN_MANIFEST:-reports/native_action_v8_single_clip/train.json}"
OUTPUT_DIR="${OUTPUT_DIR:-training/native_action_v8_single_clip}"
TRAIN_STEPS="${TRAIN_STEPS:-300}"
LOCAL_MOTION_WEIGHT="${LOCAL_MOTION_WEIGHT:-1.0}"
LOCAL_MOTION_FOCUS="${LOCAL_MOTION_FOCUS:-4.0}"
...
accelerate launch --config_file configs_acc/1gpu_smoke.yaml finetune.py \
  ...
  --freeze_lora true \
  --condition_mode native_trajectory --native_conditioner_version v8 \
  --native_trajectory_dim 111 \
  --native_baseline_init reports/direct_action/gate_d/run_015/checkpoints/adapter_step1000.pt \
  --native_adapter_init training/native_action_v7_delay_single_clip_nodrop_300/checkpoint-300 \
  --action_dropout_prob 0.0 \
  --action_contrastive_weight 0.0 --action_ranking_weight 0.0 \
  --action_motion_weight 0.0 \
  --action_local_motion_weight "$LOCAL_MOTION_WEIGHT" \
  --action_local_motion_focus "$LOCAL_MOTION_FOCUS" \
