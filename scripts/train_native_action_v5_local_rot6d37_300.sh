...
export MASTER_PORT="${MASTER_PORT:-29727}"
...
accelerate launch --config_file configs_acc/1gpu_smoke.yaml finetune.py \
  ...
  --freeze_lora true \
  --condition_mode native_trajectory --native_conditioner_version v5 \
  ...
  --output_dir training/native_action_v5_local_rot6d37_300 \
  ...
  --learning_rate 1e-4 --control_lr 5e-5 --max_grad_norm 1.0 \
