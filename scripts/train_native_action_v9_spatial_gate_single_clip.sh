...
export MASTER_PORT="${MASTER_PORT:-29743}"

TRAIN_MANIFEST="${TRAIN_MANIFEST:-reports/native_action_v8_single_clip/train.json}"
OUTPUT_DIR="${OUTPUT_DIR:-training/native_action_v9_spatial_gate_single_clip_300}"
TRAIN_STEPS="${TRAIN_STEPS:-300}"
SPATIAL_GATE_WEIGHT="${SPATIAL_GATE_WEIGHT:-0.1}"
...
accelerate launch --config_file configs_acc/1gpu_smoke.yaml finetune.py \
  ...
  --freeze_lora true \
  --condition_mode native_trajectory --native_conditioner_version v9 \
  ...
  --action_spatial_gate_weight "$SPATIAL_GATE_WEIGHT" \
  ...
  --learning_rate 1e-4 --control_lr 1e-4 \
