#!/usr/bin/env python3
"""Gate D run_001: full-dataset scaled 5000-step training, framework verbatim from run_011.

run_011 (gate_c) validated a two-fix training recipe on 91 train episodes / 1456 unique
full-range windows:
  1. Action normalization (z-score per-dim against train-split mean/std) before feeding the
     NativeTrajectoryEncoder.
  2. Timestep train/inference match: flow-shifted, per-patch, frame-0-zeroed timestep tensor
     (compute_flow_matching_loss_corrected), same convention as official_rollout.

This run generalizes that exact recipe to the full Gate D dataset via the 21-shard full-range
manifest (window_manifest_full_dataset.json): 1162 train episodes, 48,800 unique full-range
windows, from which a single seed=42 permutation selects the first 5000 (no wrap-around) for
5000 training steps. Two changes vs run_011, both data-specific, neither touching the
algorithm/framework:
  - Action normalization is per-task: stats.per_task["3400"] / stats.per_task["3401"], selected
    by each sample's task_id (run_011 used one global mean/std).
  - Fixed validation is the two activity-peak windows (fixed_validation_windows.json) resolved
    through the manifest's global_episode_lookup, not a hardcoded episode==11.

Architecture (hidden_dim=768, num_layers=2, num_heads=12, 16.7M params), LR=5e-4, seed, gradient
clip 1.0, corrected timestep, and the rollout protocol are all held identical to run_011.
"""
...
RUN = REPO / "reports/direct_action/gate_d/run_015"
...
CACHE = Path("/tmp/scratch/gate_d_run001_cache")  # fresh: raw actions + latents, unaffected by normalization/timestep
...
SEED = 42
SHUFFLE_SEED = 314159
LR = 5e-4  # unchanged from run_011
STEPS = 5000
MILESTONES = (1000, 2000, 3000, 4000, 5000)
FIXED_VALIDATION_TIMESTEP = 500
ROLLOUT_STEPS = 20
CFG_SCALE = 1.0
ENCODER_KWARGS = dict(hidden_dim=768, num_layers=2, num_heads=12)  # identical to run_008/009/010/011

EXPECT_UNIQUE = 48800
EXPECT_TRAIN_EPISODES = 1162
EXPECT_STEPS_EPISODES = 1144  # distinct episodes among the first 5000 seeded windows (computed)
...
def select_5000(manifest: dict) -> list[dict]:
    ...
    assert len({sample["unique_window_index"] for sample in selected}) == STEPS  # 5000 < 48800, no wrap-around
    return selected
...
def compute_flow_matching_loss_corrected(model, encoder, video_latent, img_latent, robot_trajectory,
    ...
    official_rollout already uses for inference. Verbatim from run_010/run_011."""
...
def main():
    ...
    try:
        ...
        (RUN / "artifacts/selected_5000_scaled.json").write_text(json.dumps(selected, indent=2))
        ...
        provenance = {
            "run_id": "direct_action/gate_d/run_015",
            "purpose": "full Gate D dataset (21 shards), 5000-step training with run_011's recipe (action normalization + corrected timestep signal) generalised to 48,800 unique windows across 1,162 train episodes",
            "changed_vs_run_011": ["data: 5000 seeded draws from 48,800 unique full-range windows across 1,162 train episodes (was: 91 episodes / 1456 unique windows)", "action normalization: per-task mean/std selected by task_id (was: single global train-split mean/std)", "validation: two activity-peak windows resolved via global_episode_lookup, one per task (was: hardcoded held_out episode 11)"],
            "held_fixed_vs_run_011": ["architecture (hidden_dim=768,num_layers=2,num_heads=12, 16.7M params)", "lr=5e-4", "corrected_timestep_signal", "seed=42", "gradient_clip_norm=1.0", "rollout protocol"],
        ...
        for index, path in enumerate(cache_paths, 1):
            ...
            if index % 50 == 0 or step in MILESTONES:
                val_str = ",".join(f"{t}={v:.4f}" for t, v in fixed_losses.items())
